...
const Inbox = () => {
  ...
  const startRecording = async () => {
    try {
      ...
      // Attempt to use audio/ogg with Opus codec
  ...
  const sendRecordedAudio = async () => {
    if (recordedAudioBlob && user) {
      const fileName = `audio-${Date.now()}.ogg`; // Use .ogg extension
      // For audio, we pass null for mediaCaption as WhatsApp API doesn't support it directly
      const mediaUrl = await uploadMediaToSupabase(recordedAudioBlob, fileName, 'audio/ogg'); // Use audio/ogg type
      if (mediaUrl) {
